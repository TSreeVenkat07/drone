...
def main():
    ...
    print(f"Loaded checkpoint: {args.checkpoint} with eval epsilon: {args.epsilon}")
        ...
        for ep_idx in range(args.n_episodes_per_difficulty):
            ...
            while not done:
                ...
                for i in range(4):
                    ...
                    # Apply jitter epsilon
                    if np.random.random() < args.epsilon:
                        valid = masks[key].astype(bool)
                        valid_indices = np.where(valid)[0]
                        action = int(np.random.choice(valid_indices))
                    else:
                        action = agents[i].select_action(obs[key], masks[key], explore=False)
                    actions[key] = action
    ...
    report_lines.append(f"JITTER EVALUATION REPORT (EPSILON={args.epsilon}) - MARL vs GREEDY")
    ...
    report_lines.append(f"  Det. acc:   {summary_mixed['detection_accuracy']*100:.1f}%")
    ...
    output_filename = "mixed_eval_jitter_results.txt"
