...
class AirSimWrapper:
    ...
    def proximity_override(self, drone_idx, vx, vy, positions_3d):
        ...
        px, py = drone_pos[0] + vx * 0.3, drone_pos[1] + vy * 0.3
    ...
    def step(self):
        ...
        # 5. Select actions using VDN network weights
        actions = {}
        for i in range(4):
            ...
            with torch.no_grad():
                q_vals = self.networks[i](obs_tensor, mask_tensor)
...
def run_evaluation_loop(max_steps=50):
    print("=" * 60)
    ...
    for s in range(max_steps):
        ...
        time.sleep(0.1) # Accelerated step pacing for evaluation display
    ...
    if HAS_AIRSIM and not wrapper.is_mock:
        ...
        land_tasks = [wrapper.client.landAsync(vehicle_name=name) for name in [f"drone_{i}" for i in range(4)]]
        for task in land_tasks:
            task.join()
        for name in [f"drone_{i}" for i in range(4)]:
