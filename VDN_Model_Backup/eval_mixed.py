...
def main():
    ...
    # Trackers for each difficulty separately and one for the overall mixed
    trackers = {
        "easy": MetricsTracker(),
    ...
    # List to hold details of each episode (including victim locations)
    episode_details = []
    ...
    for difficulty in difficulties:
        ...
        for ep_idx in range(args.n_episodes_per_difficulty):
            ...
            # Keep track of victim positions for this episode
            ...
            while not done:
                ...
                for i in range(4):
                    ...
                    with torch.no_grad():
                        q = agents[i].online_net(obs_t, mask_t)
    ...
    report_lines.append(f"FINAL EVALUATION REPORT - MARL vs GREEDY (BUILDING COLLAPSE)")
    report_lines.append(f"Checkpoint: {args.checkpoint}")
    report_lines.append("=" * 70)

    # 1. Print breakdown for each difficulty
    for diff in difficulties:
        ...
        report_lines.append(f"  Det. Acc:   {sum_diff['detection_accuracy']*100:.1f}% (Found: {sum_diff['avg_victims_found']:.2f}/7)")

    # 2. Print mixed summary
    ...
    report_lines.append(f"  Coverage:   MARL={summary_mixed['marl_coverage_pct']:.1f}% | Greedy={summary_mixed['greedy_coverage_pct']:.1f}% "
                        f"| Improvement: {summary_mixed['coverage_improvement_pct']:+.1f}% (target: >= +30%)")
    report_lines.append(f"  Latency:    MARL={summary_mixed['marl_detection_step']:.1f}steps | Greedy={summary_mixed['greedy_detection_step']:.1f}steps "
                        f"| Reduction: {summary_mixed['latency_reduction_pct']:+.1f}% (target: -20%)")
    report_lines.append(f"  Collisions: MARL={summary_mixed['avg_collisions']:.2f}/ep | Greedy={greedy_mixed_col:.2f}/ep (target MARL: 0-2)")
    report_lines.append(f"  Det. acc:   {summary_mixed['detection_accuracy']*100:.1f}% (target: >=90%)")
    ...
    # 3. Print Victim Locations log
    report_lines.append("\n" + "=" * 70)
    report_lines.append("DETAILED EPISODE LOG & VICTIM LOCATIONS")
    report_lines.append("=" * 70)
    for detail in episode_details:
        report_lines.append(
            f"Difficulty: {detail['difficulty']} | Ep: {detail['episode_index']} | "
            f"Victims Location Grid Coordinate: {detail['victim_positions']} | "
            f"MARL Found: {detail['marl_victims_found']} | "
            f"MARL Col: {detail['marl_collisions']} | MARL Cov: {detail['marl_coverage']:.1f}% | "
            f"Greedy Cov: {detail['greedy_coverage']:.1f}%"
        )
    report_text = "\n".join(report_lines)
    print(report_text[:1000] + "\n... (truncated output for console) ...")

    # Save report to a text file
    output_filename = "mixed_eval_results.txt"
